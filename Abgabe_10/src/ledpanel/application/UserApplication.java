...
public class UserApplication {
    ...
    private void lightShow() {
        for (int i = 0; i < 100; i++) {
            ...
            api.directHardwareAccess().matrix0[0] = (byte) 0B10000000;
            api.directHardwareAccess().matrix4[7] = (byte) 0B00000001;
            api.waitFor(250);

            api.directHardwareAccess().matrix0[0] = 0;
            api.directHardwareAccess().matrix4[7] = 0;
            api.waitFor(250);

            api.testLEDPanel();
            api.waitFor(250);

            api.showLEDs(new int[]{0, 39, 280, 319}, 250);
            api.waitFor(250);
            ...
            api.showStringImage(new StringImage("Hallo").toString(), 500);
            api.waitFor(250);

            api.showMovingStringImage(new MovingStringImage("Herzlich Willkommen!").movingStrings(), 20, 1);
            api.waitFor(250);
        }
    }
}
